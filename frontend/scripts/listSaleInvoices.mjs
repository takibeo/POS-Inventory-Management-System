...
(async function(){
  try {
    ...
    const resp = await fetchJson('/sales?page=0&size=50', token);
    ...
    console.log('Sale invoices count:', Array.isArray(list)?list.length:0);
    if (Array.isArray(list)) {
      list.slice(0,50).forEach(si => console.log(si.invoiceNumber || si.invoice_number || si.invoice_number, '-', si.status || ''));
    } else {
