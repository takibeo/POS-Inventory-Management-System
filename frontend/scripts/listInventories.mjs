...
(async function(){
  try {
    ...
    const res = await fetch(`${BASE}/inventories`, { headers: { Authorization: `Bearer ${token}` } });
    const data = await res.json();
    console.log('Status', res.status);
    console.log('Inventories count:', Array.isArray(data)?data.length:(data?.data?data.data.length:0));
