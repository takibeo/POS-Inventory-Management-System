...
import { Button, ConfirmModal, DataTable, type DataTableColumn, PageHeader, TableRowActions } from '../components/ui';
...
export default function PurchaseOrdersPage() {
  ...
  const formItemsColumns: DataTableColumn<FormItem>[] = [
    { key: 'productId', header: 'Sản phẩm', render: (row) => getProductName(row.productId) },
    { key: 'quantity', header: 'Số lượng' },
    { key: 'cost', header: 'Giá nhập', render: (row) => formatCurrency(row.cost) },
  ];
  ...
  const addItemToForm = () => {
    const newItem: FormItem = { productId: '', quantity: 1, cost: 0 };
  ...
  const updateFormItem = (index: number, key: keyof FormItem, value: any) => {
    setForm((prev) => ({
      ...prev,
