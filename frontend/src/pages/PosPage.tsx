...
export default function PosPage() {
  ...
  const addToCart = (product: Product) => {
    setCart((prev) => {
      ...
      return [...prev, { productId: product.id, productName: product.name, quantity: 1, unitPrice: product.price, discount: 0 }];
    });
    setSearch('');
    setShowSuggestions(false);
  };
  ...
  const updateQuantity = (productId: string, quantity: number) => {
    setCart((prev) => prev.map((item) => (item.productId === productId ? { ...item, quantity } : item)).filter((item) => item.quantity > 0));
  };

  const updateDiscount = (productId: string, discountValue: number) => {
    setCart((prev) => prev.map((item) => (item.productId === productId ? { ...item, discount: discountValue } : item)));
  };
  ...
  return (
    <div className="space-y-6 print:bg-white print:text-black">
      <PageHeader title="POS bán hàng" description="Tìm sản phẩm, thêm vào giỏ và tạo hóa đơn từ backend thực tế." />
      ...
      <div className="grid gap-6 xl:grid-cols-[1.4fr_1fr] print:hidden">
          ...
          <div className="ui-card space-y-4">
                  ...
                  <div className="absolute z-20 mt-2 w-full overflow-hidden rounded-xl border border-slate-200 bg-white shadow-lg">
                    {suggestionProducts.map((product) => (
                      ...
                      >
                        ...
                        <span className="text-sm font-semibold text-slate-900">{formatMoney(product.price)} đ</span>
                      </button>
                    ))}
                  </div>
                )}
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {filteredProducts.map((product) => (
                <button type="button" key={product.id} onClick={() => addToCart(product)} className="rounded-xl border border-slate-200 bg-white p-4 text-left shadow-sm transition hover:border-slate-900">
                  <p className="font-semibold">{product.name}</p>
                  <p className="text-sm text-slate-500">SKU: {product.sku}</p>
                  <p className="mt-2 text-sm font-medium text-slate-900">{formatMoney(product.price)} đ</p>
                </button>
              ))}
              {filteredProducts.length === 0 && <p className="text-sm text-slate-500">Không tìm thấy sản phẩm phù hợp.</p>}
            </div>
          </div>
        </div>
        ...
        <div className="ui-card space-y-4 self-start">
              ...
              {cart.map((item) => (
                <div key={item.productId} className="rounded-xl border border-slate-200 p-3">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <button type="button" className="text-sm text-red-600" onClick={() => removeItem(item.productId)}>Xóa</button>
                  </div>
          ...
          <Button type="button" className="w-full" disabled={isSubmitting || createSaleMutation.isPending} onClick={handleSubmit(onSubmit)}>
            Hoàn tất thanh toán
          </Button>
        </div>
      </div>
      ...
      {lastInvoice && (
        <div ref={printRef} className="ui-card space-y-4 print:block">
          ...
          <div className="grid gap-4 rounded-lg bg-slate-50 p-4 print:bg-transparent">
            <div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {lastInvoice.items.map((item, index) => {
                ...
                return (
                  <div key={`${item.productId}-${index}`} className="flex items-center justify-between border-b border-slate-200 py-2">
                    <div>
                      <p className="font-medium">{cartItem?.productName ?? item.productId}</p>
                      <p className="text-sm text-slate-500">SL: {item.quantity} x {formatMoney(item.unitPrice)} đ</p>
                    </div>
                    <p className="font-semibold">{formatMoney(item.quantity * item.unitPrice - (item.discount ?? 0))} đ</p>
                  </div>
                );
              })}
            </div>
